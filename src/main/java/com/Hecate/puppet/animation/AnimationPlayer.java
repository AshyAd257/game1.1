...
public class AnimationPlayer {
    ...
    public AnimationPlayer(Skeleton skeleton, PuppetRenderer puppetRenderer) {
    ...
    private void applySingleAnimation(float time) {
        ...
        try {
            ...
            for (String boneName : currentClip.getBoneNames()) {
                ...
                if (bone != null) {
                    ...
                    if (bone.isSwingEnabled()) {
                        float swingAngle = bone.calculateSwingAngle(time);
                        Vector3f swingAxis = bone.getSwingAxis();

                        // 创建swing旋转四元数
                        Quaternion swingRotation = new Quaternion();
                        swingRotation.fromAngleAxis(swingAngle, swingAxis);

                        // 叠加到当前旋转上
                        Quaternion currentRotation = bone.getLocalRotation();
                        Quaternion finalRotation = currentRotation.mult(swingRotation);
                        bone.setLocalRotation(finalRotation);
                    }
                }
            ...
            for (String boneName : currentClip.getBoneNames()) {
                Bone bone = skeleton.findBone(boneName);
                if (bone != null && bone.isSwingEnabled()) {
                    bone.updateDirectionFromRotation();
                }
            }
