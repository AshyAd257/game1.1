...
public class Bone {
    ...
    // FreeBone目标位置（不包含物理摆动，用于shadowCaster）
    // 当骨骼类型为FREE时，此位置是物理模拟的目标位置（无物理偏移）
    // 当骨骼类型为CONNECTED时，此位置与localPosition相同
    private Vector3f targetLocalPosition;
    ...
    // 摇摆开关 - 是否启用摇摆效果
    private boolean swingEnabled = false;  // 默认关闭

    // 摇摆轴 - 摇摆旋转的轴向（归一化向量）
    // 例如：(0, 0, 1) 表示绕Z轴摇摆（左右摆动）
    //       (1, 0, 0) 表示绕X轴摇摆（前后摆动）
    //       (0, 1, 0) 表示绕Y轴摇摆（扭转）
    private Vector3f swingAxis = new Vector3f(0, 0, 1);  // 默认绕Z轴（左右摆动）

    // 摇摆幅度 - 最大摇摆角度（度）
    // 例如：30.0f 表示最大摆动±30度
    private float swingAmplitude = 15.0f;  // 默认±15度
    ...
    // ==================== 分组系统 (Bone Grouping System) ====================

    // 所属的骨骼组（可选，用于整体旋转等操作）
    private String groupId;  // 组ID（null表示不属于任何组）
    ...
    public Bone(String name) {
        ...
        // 初始化目标位置（默认等于localPosition）
        this.targetLocalPosition = localPosition.clone();
    ...
    /**
     * 计算世界空间变换矩阵（使用targetLocalPosition，忽略FreeBone物理影响）
     * 【用于shadowCaster】避免FreeBone物理摆动在旋转后的shadowCaster上可见
     *
     * targetLocalPosition说明：
     * - 对于CONNECTED类型骨骼：targetLocalPosition与localPosition相同（包含动画）
     * - 对于FREE类型骨骼：targetLocalPosition是物理模拟前的目标位置（动画但不含物理摆动）
     */
    public void getTargetWorldTransform(Vector3f outPosition, Quaternion outRotation, Vector3f outScale) {
        if (parent == null) {
            // 根骨骼，使用目标位置
            outPosition.set(targetLocalPosition);
            outRotation.set(localRotation);
            outScale.set(localScale);
        } else {
            // 递归计算父骨骼的世界变换（父骨骼也使用target位置）
            Vector3f parentWorldPos = new Vector3f();
            Quaternion parentWorldRot = new Quaternion();
            Vector3f parentWorldScale = new Vector3f();
            parent.getTargetWorldTransform(parentWorldPos, parentWorldRot, parentWorldScale);

            // 组合变换（使用targetLocalPosition而不是localPosition）
            Vector3f scaledPos = targetLocalPosition.mult(parentWorldScale);
            Vector3f rotatedPos = parentWorldRot.mult(scaledPos);
            outPosition.set(parentWorldPos.add(rotatedPos));
    ...
    public void setLocalPosition(Vector3f localPosition) {
        ...
        // 对于CONNECTED类型骨骼，targetLocalPosition始终与localPosition同步
        if (this.boneType == BoneType.CONNECTED) {
            this.targetLocalPosition = localPosition.clone();
        }
    }

    public Vector3f getTargetLocalPosition() {
        return targetLocalPosition;
    }

    public void setTargetLocalPosition(Vector3f targetLocalPosition) {
    ...
    public float calculateSwingAngle(float time) {
        ...
        float angleInDegrees = swingAmplitude * FastMath.sin(
            FastMath.TWO_PI * swingFrequency * time + swingPhaseOffset
        );
        return angleInDegrees * FastMath.DEG_TO_RAD;  // 转换为弧度
    }

    // ==================== Camera Follow 相机跟随自由度 Getters/Setters ====================
    ...
    /**
     * 设置水平方向相机跟随自由度
     * @param cameraFollowFreedomX 水平方向相机跟随自由度
     */
    ...
    /**
     * 设置垂直方向相机跟随自由度
     * @param cameraFollowFreedomY 垂直方向相机跟随自由度
     */
    ...
    public void updateDirectionFromRotation() {
        if (!isMultiDirectionTextureEnabled()) {
            return;  // 单贴图模式不需要切换
        }
    ...
    // ==================== 分组系统 Getters/Setters ====================

    /**
     * 获取所属的组ID
     * @return 组ID，如果不属于任何组返回null
     */
    ...
    /**
     * 设置所属的组ID
     * @param groupId 组ID（null表示不属于任何组）
     */
    ...
    public boolean isInGroup(String groupId) {
        return this.groupId != null && this.groupId.equals(groupId);
    }
}
