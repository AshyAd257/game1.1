 ...
 * 管理整个木偶的渲染，包括所有骨骼部件
 */
public class EditorPuppetRenderer {
    ...
    private final EditorSkeleton skeleton;
    private final Node puppetNode;

    private final Map<String, EditorPuppetPartRenderer> partRenderers;
    private final List<EditorPuppetPartRenderer> allRenderers;
    ...
    // 自由骨骼物理系统
    private final Map<String, EditorFreeBonePhysics> freeBonePhysics;
    ...
    // 相机角度偏移（用于Live2D风格的部件晃动）
    // X = 水平角度偏移（-1到1，负值=相机在左，正值=相机在右）
    // Y = 垂直角度偏移（-1到1，负值=相机在下，正值=相机在上）
    private Vector3f cameraAngleOffset = new Vector3f(0, 0, 0);

    // 相机角度偏移的平滑系数（0-1，越大越平滑）
    private float cameraAngleSmoothness = 0.9f;

    public EditorPuppetRenderer(SimpleApplication app, EditorSkeleton skeleton) {
        this.app = app;
    ...
    public void initialize() {
        ...
        for (EditorBone bone : skeleton.getAllBones()) {
            EditorPuppetPartRenderer partRenderer = new EditorPuppetPartRenderer(
                    app,
                    bone,
                    puppetNode,
                    defaultPartWidth,
            ...
            // 如果是自由骨骼，创建物理系统
            if (bone.isFreeBone()) {
                EditorFreeBonePhysics physics = new EditorFreeBonePhysics(bone);
                // 应用骨骼中保存的物理参数
    ...
    public void update(float tpf) {
        ...
        // 【Live2D风格相机跟随】更新相机角度偏移
        updateCameraAngleOffset();
        ...
        // 更新自由骨骼的物理模拟
        for (EditorFreeBonePhysics physics : freeBonePhysics.values()) {
            physics.update(tpf);
        }

        // 更新所有部件的世界变换
        for (EditorPuppetPartRenderer renderer : allRenderers) {
            renderer.updateTransform(tpf);
        }
    ...
    /**
     * 更新相机角度偏移（Live2D风格相机跟随）
     * 计算相机相对于木偶的角度变化，用于部件晃动效果
     */
    private void updateCameraAngleOffset() {
        if (app == null || app.getCamera() == null) {
            return;
        }

        // 获取当前相机位置
        Vector3f currentCameraPos = app.getCamera().getLocation();

        // 如果是第一帧，初始化上一帧位置
        if (lastCameraPosition == null) {
            lastCameraPosition = currentCameraPos.clone();
            return;
        }

        // 计算相机移动向量
        Vector3f cameraMovement = currentCameraPos.subtract(lastCameraPosition);

        // 获取木偶位置
        Vector3f puppetPos = puppetNode.getWorldTranslation();

        // 计算从木偶指向相机的方向向量
        Vector3f toCam = currentCameraPos.subtract(puppetPos);
        ...
        // 计算水平方向（左右）的角度偏移
        // 使用相机移动在水平面上的投影
        Vector3f horizontalMovement = new Vector3f(cameraMovement.x, 0, cameraMovement.z);
        ...
        if (horizontalMovement.lengthSquared() > 0.0001f) {
            // 使用叉积判断方向
            Vector3f right = Vector3f.UNIT_Y.cross(toCam).normalizeLocal();
            horizontalAngle = horizontalMovement.dot(right);
        }

        // 计算垂直方向（上下）的角度偏移
        float verticalAngle = cameraMovement.y;

        // 归一化到 -1 到 1 范围（使用合理的缩放因子）
        float maxMovement = 0.5f; // 相机移动0.5单位对应最大偏移
        float targetOffsetX = Math.max(-1f, Math.min(1f, horizontalAngle / maxMovement));
        float targetOffsetY = Math.max(-1f, Math.min(1f, verticalAngle / maxMovement));

        // 应用平滑插值（避免突然跳变）
        cameraAngleOffset.x = cameraAngleOffset.x * cameraAngleSmoothness +
                              targetOffsetX * (1f - cameraAngleSmoothness);
        cameraAngleOffset.y = cameraAngleOffset.y * cameraAngleSmoothness +
                              targetOffsetY * (1f - cameraAngleSmoothness);
    ...
    private void calculateAndSetCameraDirection() {
        ...
        for (EditorBone bone : skeleton.getAllBones()) {
            ...
            // 获取部件渲染器并更新贴图
            EditorPuppetPartRenderer partRenderer = partRenderers.get(bone.getName());
     ...
     * 获取指定骨骼的部件渲染器
     */
    public EditorPuppetPartRenderer getPartRenderer(String boneName) {
     ...
     * 添加部件渲染器
     */
    public void addPartRenderer(EditorBone bone) {
     ...
     * 添加部件渲染器（指定尺寸）
     */
    public EditorPuppetPartRenderer addPartRenderer(EditorBone bone, float width, float height) {
        ...
        // 【修复】在设置 parentRenderer 后立即更新变换，确保 billboard 正确应用
        // 这对新创建的模型尤其重要
        partRenderer.updateTransform(0f); // 初始化时传入0
    ...
    public void removePartRenderer(String boneName) {
        EditorPuppetPartRenderer renderer = partRenderers.get(boneName);
        if (renderer == null) {
            return;
        }
        ...
        for (EditorBone bone : skeleton.getAllBones()) {
            ...
            if (parent != null) {
                ...
                if (lineGeometry != null) {
                    ...
                    EditorPuppetPartRenderer childRenderer = partRenderers.get(bone.getName());
                    EditorPuppetPartRenderer parentRenderer = partRenderers.get(parent.getName());
     ...
     * 创建骨骼连接线
     */
    private void createBoneConnectionLine(EditorBone child, EditorBone parent) {
        // 从PuppetPartRenderer获取最终世界位置（包括offset和自定义旋转）
        EditorPuppetPartRenderer childRenderer = partRenderers.get(child.getName());
        EditorPuppetPartRenderer parentRenderer = partRenderers.get(parent.getName());
     ...
     * @param bone 要添加物理的骨骼
     */
    public void addFreeBonePhysics(EditorBone bone) {
     ...
     * @param bone 要移除物理的骨骼
     */
    public void removeFreeBonePhysics(EditorBone bone) {
        if (bone == null) {
            return;
        }
        freeBonePhysics.remove(bone.getName());
    ...
    private void sortByPriority() {
        ...
        for (EditorPuppetPartRenderer renderer : allRenderers) {
            EditorBone bone = renderer.getBone();
    ...
    private void updateShadowCastersWithLightDir(Vector3f lightDir) {
        ...
        for (EditorPuppetPartRenderer renderer : allRenderers) {
            ...
            if (shadowCaster != null) {
                ...
                // 【修复】让shadow caster使用统一billboard旋转，始终面向摄像机并保持竖直
                // 这样shadow caster就像puppet部件一样，始终竖直站立并面向摄像机
    ...
    public List<EditorPuppetPartRenderer> getAllPartRenderers() {
    ...
    /**
     * 获取相机角度偏移（用于Live2D风格部件晃动）
     * @return 相机角度偏移向量（X=水平，Y=垂直，范围-1到1）
     */
    public Vector3f getCameraAngleOffset() {
        return cameraAngleOffset.clone();
    }

    /**
     * 转换为基础 PuppetRenderer 类型（用于导出）
     * 将 EditorPuppetRenderer 转换为游戏运行时的 PuppetRenderer
     * 注意：这个方法主要用于导出，只复制基本结构，不包括所有运行时状态
     */
    public com.Hecate.puppet.core.PuppetRenderer getBaseRenderer() {
        com.Hecate.puppet.core.PuppetRenderer renderer = new com.Hecate.puppet.core.PuppetRenderer(
            app, skeleton.getBaseSkeleton()
        );

        // 转换 BillboardMode 枚举
        com.Hecate.puppet.core.PuppetRenderer.BillboardMode baseBillboardMode;
        switch (this.billboardMode) {
            case DISABLED:
                baseBillboardMode = com.Hecate.puppet.core.PuppetRenderer.BillboardMode.DISABLED;
                break;
            case UNIFIED:
                baseBillboardMode = com.Hecate.puppet.core.PuppetRenderer.BillboardMode.UNIFIED;
                break;
            case INDEPENDENT:
                baseBillboardMode = com.Hecate.puppet.core.PuppetRenderer.BillboardMode.INDEPENDENT;
                break;
            default:
                baseBillboardMode = com.Hecate.puppet.core.PuppetRenderer.BillboardMode.UNIFIED;
        }
        renderer.setBillboardMode(baseBillboardMode);

        // 转换所有部件渲染器（复制完整配置）
        for (EditorPuppetPartRenderer editorPartRenderer : allRenderers) {
            EditorBone editorBone = editorPartRenderer.getBone();
            ...
            if (baseBone != null) {
                ...
                if (offset != null) {
                    basePartRenderer.setOffset(offset.x, offset.y);
                    basePartRenderer.setOffsetZ(offset.z);
                }

                // 复制自定义旋转
                basePartRenderer.setCustomRotationX(editorPartRenderer.getCustomRotationX());
                basePartRenderer.setCustomRotationZ(editorPartRenderer.getCustomRotationZ());

                // 复制中心点
                Vector3f pivotPoint = editorPartRenderer.getPivotPoint();
                if (pivotPoint != null) {
                    basePartRenderer.setPivotPoint(pivotPoint.clone());
                }

                // 复制UV坐标
                basePartRenderer.setUV(
                    editorPartRenderer.getUvOffsetX(),
                    editorPartRenderer.getUvOffsetY(),
                    editorPartRenderer.getUvScaleX(),
                    editorPartRenderer.getUvScaleY()
                );

                // 加载纹理（从bone获取，因为bone已经有完整的纹理信息）
                basePartRenderer.updateTextureFromBone();
            }
        }
